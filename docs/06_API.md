...
## أنماط إلزامية عند مخاطبة PostgREST (منذ 27 أغسطس 2026)
* **القراءة:** كل نداءات GET تمر عبر `getJson(url)` التي تفرض `cache: 'no-store'` — لمنع كاش WebView من عرض بيانات قديمة بعد التقاط/تسليم تم فعلاً.
* **فلتر `in.(...)`:** يُبنى حصراً عبر `inFilterList(values)`. **`encodeURIComponent` وحده غير كافٍ** لأنه لا يُرمّز الأقواس، فقيمة تحوي `)` تُغلق قائمة الفلتر وتُنتج خطأ 400 صامتاً. الدالة تقتبس كل قيمة باقتباسين مزدوجين (توصية PostgREST) وتُهرّب `\` و`"`.
* **قياس نجاح الكتابة:** بـ`res.ok` أولاً، لا بنجاح `res.json()`. استجابة 2xx بجسم غير مقروء = الكتابة وصلت؛ مصفوفة فاضية = الفلتر لم يطابق أي صف (سبب مختلف تماماً).
